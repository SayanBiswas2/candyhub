...
const Sidebar = ({page,setPage,sideBarOppend,setSideBarOppend}) => {
  const router = useRouter()
  let admin = useSelector(state=>state.admin.admin)
  let permission = useSelector(state=>state.admin.permission)

  return (
    <>
    {/* <div className='h-[100vh] w-[40px]'></div> */}
    <div
      className={`${sideBarOppend ? 'w-[40vw] md:w-[250px]' : 'w-[50px] md:w-[60px]'} ${admin ? 'fixed' : 'hidden'} bg-color-second text-color-font h-[100vh] pt-4 px-[5px] md:px-[10px] duration-500  mt-0 top-12 z-50`}
      >
        ...
        <div
          className={`w-4 h-4 rounded-full shadow-md absolute bg-color-second ${sideBarOppend ? 'left-[40vw] md:left-[250px] -rotate-180' : 'left-[50px] md:left-[60px]'}  translate-x-[-50%] top-4 duration-500 cursor-pointer`}
